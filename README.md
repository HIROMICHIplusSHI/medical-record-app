...
- **施術記録**: 患者ごとの施術履歴・問診票・同意書（電子署名 + PDF 出力）
...
「カルテ（診療録）」は患者ごとの記録の束を指す語なので、このアプリでは **患者詳細ページがカルテ本体**にあたり、
そこに施術1回分の記録が時系列で並ぶ構造にしています。

| UI 表記 | 実体 | モデル |
|---|---|---|
| 施術記録 | 1来院・1施術分の記録（来院日・施術内容・コスト明細・写真・同意書） | `MedicalRecord` |
| 施術履歴 | ある患者の施術記録の集合、および全患者横断の一覧 | `MedicalRecord` の集合 |
| 問診票 | 患者が記入する健康状態・基本情報（暗号化） | `Questionnaire` |

`MedicalRecord` というモデル名は初期実装時の命名で、実体は「施術記録1件」です。
UI 表記を実体に合わせて統一し、テーブル名は互換性のため据え置いています。
...
- **PostgreSQL**: 施術記録の柔軟なデータ構造を `jsonb` で扱いつつ、Active Record Encryption と
  組み合わせて個人情報を保護。
