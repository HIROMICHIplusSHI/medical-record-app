...
RSpec.describe 'UserDashboards', type: :request do
  ...
  describe 'POST /dashboard/dismiss_announcement' do
    let(:announcement) { create(:announcement, :published, author: admin, published_at: 1.day.ago) }

    context '未認証の場合' do
      it 'ログインページにリダイレクトされる' do
        post user_dashboard_dismiss_announcement_path, params: { announcement_id: announcement.id }
        expect(response).to redirect_to(new_user_session_path)
      end
    end
    ...
    context '認証済みの場合' do
      ...
      context 'エラーケース' do
        it '存在しないお知らせIDの場合、404を返す' do
          post user_dashboard_dismiss_announcement_path, params: { announcement_id: 99_999 }
          expect(response).to have_http_status(:not_found)
        end

        it '非公開のお知らせの場合、403を返す' do
          draft_announcement = create(:announcement, author: admin, status: :draft)
          post user_dashboard_dismiss_announcement_path, params: { announcement_id: draft_announcement.id }
        ...
        it '期限切れのお知らせの場合、403を返す' do
          ...
          post user_dashboard_dismiss_announcement_path, params: { announcement_id: expired_announcement.id }
          expect(response).to have_http_status(:forbidden)
        end
      end
      ...
      context 'セッション管理' do
        it 'セッションサイズが最大数を超えた場合、古いIDを削除する' do
          ...
          announcements.each do |ann|
            post user_dashboard_dismiss_announcement_path, params: { announcement_id: ann.id }
          end

          # 101個目を追加
          new_announcement = create(:announcement, :published, author: admin, published_at: 1.day.ago)
          post user_dashboard_dismiss_announcement_path, params: { announcement_id: new_announcement.id }
