...
- [x] **フィーチャーフラグの実装（2025-10-12）**
  - コントローラーに`USE_CHECKBOX_UI`フラグ追加
  - 問診票UIの切り替え機能実装（チェックボックス版 vs テキストエリア版）
  - `_form_textarea.html.erb`作成（現行UI）
  - `_form_checkbox.html.erb`作成（Phase 3用UI）
  - 現在は`USE_CHECKBOX_UI = false`でテキストエリア版が表示される
  - Phase 3でカルテページ実装後、簡単に切り替え可能

- [x] **患者検索機能の実装（2025-10-12）**
  - Patientモデルに`.search`スコープ追加
  - 氏名・電話番号で部分一致検索
  - 全角数字の自動半角変換対応
  - 暗号化フィールド対応（メモリ内フィルタリング）
  - 検索フォームUI実装（検索・クリアボタン）
  - 13の検索テストケース追加（全成功）

- [x] **ページネーション機能の実装（2025-10-12）**
  - kaminari設定追加
  - 25件/ページで表示
  - 検索結果にもページネーション適用
  - クエリパラメータ保持（検索しながらページ移動可能）
  - 3つのページネーションテスト追加（全成功）
