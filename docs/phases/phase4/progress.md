...
#### 実装範囲
- [ ] 問診票→患者への自動同期機能（`sync_to_patient`メソッド）
  - [ ] 問診票作成時に患者情報を自動更新
  - [ ] 問診票更新時に患者情報を自動更新
- [ ] 問診票新規作成時の患者情報事前入力
  - [ ] 予約時の名前・電話番号を事前入力（編集可能）
- [ ] 問診票show画面の実装
  - [ ] 読み取り専用の表示画面作成
  - [ ] JSON配列データを人間が読みやすいテキスト形式で表示
  - [ ] 印刷・PDF出力に適したレイアウト
- [ ] 患者詳細画面の表示ロジック修正
  - [ ] 問診票が存在する場合: 問診票から基本情報を表示
  - [ ] 問診票が存在しない場合: 患者レコードから表示
- [ ] 患者編集画面の調整
  - [ ] 問診票が存在する場合: 基本情報は表示のみ（編集不可）
  - [ ] 「基本情報を編集するには問診票を編集してください」というメッセージ表示
  - [ ] 問診票が存在しない場合: 基本情報編集可能
- [ ] ルーティング追加（`questionnaires#show`）
- [ ] Systemテスト作成（TDD）
  - [ ] 問診票作成時の自動同期テスト
  - [ ] 問診票更新時の自動同期テスト
  - [ ] show画面の表示テスト
