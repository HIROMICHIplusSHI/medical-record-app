module NavigationHelper
  def link_to_nav_item(text, path, disabled: false)
    css_class = if disabled
                  'whitespace-nowrap px-3 py-2 rounded-md text-sm font-medium text-gray-300 cursor-not-allowed'
                elsif current_page?(path)
                  'whitespace-nowrap px-3 py-2 rounded-md text-sm font-medium bg-greige-100 text-accent-primary'
                else
                  'whitespace-nowrap px-3 py-2 rounded-md text-sm font-medium text-greige-700 ' \
